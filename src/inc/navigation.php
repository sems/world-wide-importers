    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <?php
                ...
                if (!isset($checkForNav)) {
                    // Check if checkForNav isset, if not, equal to title
                    $checkForNav = $title;
                }
                ...
                foreach ($pages as $key => $value) {
                    if (is_array($value)) {
                        print("<li class='nav-item dropdown'>
                                <a class='nav-link dropdown-toggle".(in_array($checkForNav, $value) ? " active":"")."' href='#' id='navbarDropdown' role='button' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'>
                                    $key
                                </a>
                                <div class='dropdown-menu' aria-labelledby='navbarDropdown'>");
                        foreach ($value as $_key => $_value) {
                            print("<a class='dropdown-item".($checkForNav == $_value ? " active":"")."' href='$_key'>$_value</a>");
                        }
                        print("</div></li>");
                    } else {
                        print("<li class='nav-item ".($checkForNav == $value ? "active":"")."'>
            ...
            <form action="products.php" method="get" class="form-inline my-2 my-lg-0">
                ...
                <div>
                    ...
                    <a class="btn btn-primary my-2 my-sm-0 basketBtn <?php print($checkForNav == "Winkelwagen" ? "basketActive":""); ?>" href="basket.php"><i class="fas fa-shopping-basket basket"></i></a>
                    <?php
                        // Checks if user is logged in or not.
                        if(!isset($_SESSION['logged_in']) || ($_SESSION['logged_in'] == false)) {
                            print("<a class='btn btn-primary my-2 my-sm-0 basketBtn".($checkForNav == 'Login' ? 'basketActive':'')."' href='login.php'><i class='fas fa-sign-in-alt basket'></i></a></li>");
                            //print("<a class='btn btn-primary my-2 my-sm-0 basketBtn".($checkForNav == 'Registreer' ? 'basketActive':'')."' href='register.php'><i class='fas fa-user basket'></i></a></li>");
                        } else {
                            print("<a class='btn btn-primary my-2 my-sm-0 basketBtn".($checkForNav == 'Profiel' ? 'basketActive':'')."' href='profile.php'><i class='fas fa-user basket'></i></a></li>");
                            print("<a class='btn btn-primary my-2 my-sm-0 basketBtn".($checkForNav == 'Uitloggen' ? 'basketActive':'')."' href='logout.php'><i class='fas fa-sign-out-alt basket'></i></a></li>");
                        };
                    ?>
                </div>
            </form>
